...
VALUES  ("Astra Urtyp Gross", "1.00€", "1", DATETIME('now'),  ?);""", (_uid,))

    connection.commit()
except (connection.Error, connection.Warning) as e:
    print(e)
finally:
    print ("1 großes Astra Urtyp wurde erfolgreich gebucht")
    print ("------------------------------------------------------")
...
sql_command = """ UPDATE lager SET bestand = bestand - 1 WHERE getraenke_name = "Astra Urtyp Gross" """
